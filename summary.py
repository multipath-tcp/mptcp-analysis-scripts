...
def time_completion_big_connections_new(log_file=sys.stdout, min_bytes=15000000, max_bytes=25000000):
    results = {co.S2D: {}, co.D2S: {}}
    results_two = {co.S2D: {}, co.D2S: {}}
    base_graph_name_bytes = "boxplot_duration_" + args.app + "_" + start_time + '_' + stop_time
    base_graph_path_bytes = os.path.join(sums_dir_exp, base_graph_name_bytes)
    ...
    for fname, data in connections.iteritems():
        ...
        if 'mptcp_fm_' in condition:
            ...
            if 'wlan' in condition and ((20150214 <= fname_date and fname_date <= 20150220) or (20150326 <= fname_date and 20150327 >= fname_date) or fname_date == 20150330):
                key = 'WiFi'
            elif 'wlan' in condition and ((20150304 <= fname_date and fname_date <= 20150308) or fname_date == 20150329):
                key = 'WiFi 1M'
        ...
        if key:
            for conn_id, conn in data.iteritems():
                if isinstance(conn, tcp.TCPConnection):
                    for direction in co.DIRECTIONS:
                        if direction in conn.flow.attr:
                            if conn.flow.attr[direction][co.BYTES] >= min_bytes and conn.flow.attr[direction][co.BYTES] <= max_bytes:
                                results[direction][key].append(conn.flow.attr[co.DURATION])
                                results_two[direction][key].append((conn.flow.attr[direction][co.BYTES] + 0.0) / conn.flow.attr[co.DURATION])
                elif isinstance(conn, mptcp.MPTCPConnection):
                    for direction in co.DIRECTIONS:
                        if direction in conn.attr:
                            if co.BYTES_MPTCPTRACE in conn.attr[direction] and conn.attr[direction][co.BYTES_MPTCPTRACE] >= min_bytes and conn.attr[direction][co.BYTES_MPTCPTRACE] <= max_bytes:
                                results[direction][key].append(conn.attr[co.DURATION])
                                results_two[direction][key].append((conn.attr[direction][co.BYTES_MPTCPTRACE] + 0.0) / conn.attr[co.DURATION])
...
def textual_summary(log_file=sys.stdout):
    ...
    for fname, conns in connections.iteritems():
        ...
        if condition not in data.keys():
            data[condition] = {'<1s': 0, ">=1s<10K": 0, ">=1s>=10K": 0, "<9s": 0, ">=9s<10K": 0, ">=9s>=10K": 0}
            count[condition] = {'<1s': 0, ">=1s<10K": 0, ">=1s>=10K": 0, "<9s": 0, ">=9s<10K": 0, ">=9s>=10K": 0}
        ...
        for conn_id, conn in conns.iteritems():
            ...
            if duration < 1:
                ...
                if nb_bytes_s2d + nb_bytes_d2s < 10000:
                    data[condition][">=1s<10K"] += nb_bytes_s2d + nb_bytes_d2s
                    count[condition][">=1s<10K"] += 1
                else:
                    data[condition][">=1s>=10K"] += nb_bytes_s2d + nb_bytes_d2s
                    count[condition][">=1s>=10K"] += 1
...
def cdf_overhead_retrans_reinj_new(log_file=sys.stdout):
        ...
        if 'mptcp_fm' in condition and 'both' in condition:
            if condition not in results[co.S2D]:
                for direction in co.DIRECTIONS:
                    ...
                    max_overhead_tot_bytes[direction][condition] = 0.
            ...
            for conn_id, conn in data.iteritems():
                ...
                for direction in co.DIRECTIONS:
                    ...
                    if reinj_data_bytes[direction] > 0:
                        ...
                        old = max_overhead_bytes[direction][condition]
                        max_overhead_bytes[direction][condition] = max(max_overhead_bytes[direction][condition], reinj_data_bytes[direction])
                        if old < max_overhead_bytes[direction][condition]:
                            max_overhead_tot_bytes[direction][condition] = total_data_bytes[direction]
    ...
    for direction in results:
        for condition in results[direction]:
            ...
            tot_graph_full_path = graph_full_path + "_details_" + direction + "_" + condition + ".pdf"
            co.plot_line_graph(to_plot, ['Total', 'Reinjections'], ['b', 'r'], 'Connections', 'Number of data bytes', '', tot_graph_full_path, y_log=True)
            print(condition, direction, number_overhead[direction][condition], number_total[direction][condition], number_overhead[direction][condition] / number_total[direction][condition], max_overhead_bytes[direction][condition], max_overhead_tot_bytes[direction][condition], max_overhead_frac[direction][condition], file=log_file)
