                    ...
                    for flow_id, flow in conn.flows.iteritems():
                        if co.REINJ_ORIG_TIMESTAMP in flow.attr[direction] and co.START in flow.attr:
                            for ts in flow.attr[direction][co.REINJ_ORIG_TIMESTAMP]:
                                ...
                                if direction == co.D2S and ts_fix / duration >= 0.28 and ts_fix / duration <= 0.35:
                                    print("TO LOOK AT", fname, conn_id, flow_id, ts_fix / duration, ts, start_time, ts_fix, duration, file=log_file)
...
def delay_mpcapable_mpjoin_quantify_handover(log_file=sys.stdout, threshold_handover=1.0):
    ...
    co.plot_cdfs_natural({'multiflow': {'delta': syn_additional_sfs}}, ['red'], 'Seconds', base_graph_path, xlog=True)
